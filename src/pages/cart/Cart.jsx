...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: "rzp_test_gk4fRt6swVe9Ol",
      key_secret: "CAzmY7bsRj9xfsJCBgDjKIpL",
      ...
      handler: async function (response) {
        ...
        try {
          ...
          console.log(error);
          toast.error('Failed to store order information.');
        }
      },
      theme: {
        color: "#3399cc"
  ...
  return (
    <Layout >
      <div className="h-screen bg-gray-100 pt-5 mb-[65%]" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          ...
          {cartItems.length > 0 && (
            <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
              <div className="mb-4">
                <h2 className="text-lg font-bold" style={{ color: mode === 'dark' ? 'white' : '' }}>Order Summary</h2>
                <ul>
                  {cartItems.map((item, index) => (
                    <li key={index} className="flex justify-between text-sm my-2">
                      <span style={{ color: mode === 'dark' ? 'white' : '' }}>{item.title}</span>
                      <span style={{ color: mode === 'dark' ? 'white' : '' }}>₹{item.price}</span>
                    </li>
                  ))}
                </ul>
              </div>
